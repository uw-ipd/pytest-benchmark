...
def pytest_report_header(config):
    ...
    return (
        ...
        ")"
    ).format(bs.options, version=__version__, timer=bs.options.get("timer"))
...
def add_histogram_options(addoption, prefix="benchmark-"):
    ...
    addoption(
        ...
        " slashes ('/') then directories will be created. Default: %r"
        % filename_prefix,
    )
...
def pytest_benchmark_compare_machine_info(
    ...
    if compared_machine_info != machine_info:
        benchmarksession.logger.warn(
            ...
            % (machine_info, compared_machine_info, benchmarksession.storage.location)
        )
...
def pytest_benchmark_group_stats(config, benchmarks, group_by):
    ...
    for grouped_benchmarks in groups.values():
        grouped_benchmarks.sort(
            key=operator.itemgetter("fullname" if "full" in group_by else "name")
        )
...
def benchmark(request):
    ...
    if bs.skip:
        ...
        fixture = BenchmarkFixture(
            ...
            post_run_hooks=[
                lambda fixture, f: request.config.hook.pytest_benchmark_fixture_post_run(
                    config=request.config, fixture=fixture, f=f
                )
            ],
    ...
    if marker:
        ...
        for name in marker.kwargs:
            if name not in (
                ...
                raise ValueError(
                    "benchmark mark can't have %r keyword argument." % name
                )
